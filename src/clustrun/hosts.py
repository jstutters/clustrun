...
class Host:
    ...
    def __repr__(self):
        return "Host({0!r}, port={1}, n_jobs={2})".format(
            self.hostname, self.port, self.n_jobs
        )

    def to_dict(self):
        return {"hostname": self.hostname, "port": self.port, "n_jobs": self.n_jobs}


def read_hosts_file(hosts_file):
    exp = re.compile(
        r"^(?P<hostname>\S*){1}\s*(port=(?P<port>[0-9]+))?\s*(n=(?P<n_jobs>[0-9]+))?"
    )
    host_list = [exp.match(l.strip()) for l in hosts_file.readlines()]
    hosts = []
    for h in host_list:
        host_args = {"hostname": h.group("hostname")}
        if h.group("port"):
